...
- Передача материалов на утверждение специалисту команды
...
stateDiagram-v2
    ...
    awaiting_approval --> awaiting_client_signoff: назначено согласование с компанией
    ...
    awaiting_client_signoff --> approved: согласовано компанией
    awaiting_client_signoff --> drafting: возвращено компанией: текст
    awaiting_client_signoff --> generating: возвращено компанией: визуальный ряд
...
| `awaiting_approval` | Материал ожидает утверждения специалистом команды |
| `escalated` | Материал передан человеку без автоматического заключения |
| `awaiting_client_signoff` | Материал утверждён специалистом и ожидает согласования с компанией |
...
Каждый материал перед публикацией утверждается специалистом команды. Переход в состояние `approved` выполняется только из состояний `awaiting_approval` и `awaiting_client_signoff`, то есть только по решению человека. Переход в состояние `publishing` выполняется исключительно из состояния `approved`.
...
При достижении предельного значения дальнейшие циклы автоматической доработки материала не запускаются: материал переводится в состояние `escalated` и передаётся специалисту команды.

Возврат материала на доработку специалистом команды или компанией в счётчик циклов автоматической доработки не входит.
