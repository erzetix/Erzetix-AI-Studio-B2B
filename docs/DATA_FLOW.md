...
| `MaterialView` | Рабочее место команды | Интерфейс сотрудника | Представление материала в контуре утверждения |
...
Решение команды фиксируется в виде объекта `ApprovalDecision`. Для материалов, по которым назначено согласование с компанией, решение компании, полученное вне комплекса, фиксируется сотрудником команды отдельным объектом `ApprovalDecision` со значением `source` = `client`.
...
По показателям эффективности за период сотрудник команды формирует объект `ClientReport`, который направляется компании вне комплекса.
