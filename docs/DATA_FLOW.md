...
| `ApprovalDecision` | Рабочее место команды | API-оркестратор | Решение специалиста об утверждении материала либо возврате на доработку |
...
Специалист формирует объект `Brief` в рабочем месте команды на основании договорённостей с компанией и передаёт его API-оркестратору. Оркестратор создаёт объект `CampaignRun` в состоянии `received` и инициирует обработку.
...
Агент-визуализатор возвращает объект `AssetBundle`, содержащий сгенерированные материалы и перечень невыполненных заданий. Оркестратор собирает результаты генерации каждого материала в готовый медиафайл.
...
Число циклов автоматической доработки материала ограничено. При исчерпании лимита материал передаётся специалисту с заключением `escalated`.

### Утверждение

Каждый материал перед публикацией утверждается специалистом. Материалы с заключением `approved` и `escalated` преобразуются рабочим местом команды в объекты `MaterialView` и отображаются в контуре утверждения.

Решение специалиста фиксируется в виде объекта `ApprovalDecision`.

Решения передаются API-оркестратору и определяют переход состояния отдельного материала. Утверждение переводит материал в состояние `approved`. Возврат на доработку сопровождается замечаниями `RevisionNote` и возвращает материал агенту-креатору или агенту-визуализатору; остальные материалы кампании не затрагиваются.

### Публикация и обратная связь

Утверждённые материалы передаются в модуль публикации и размещаются специалистами в аккаунтах компаний. После размещения специалисты вводят показатели эффективности, формируется объект `PublicationMetrics`.
...
- в рабочее место команды — для автоматического формирования отчёта об эффективности `ClientReport`
